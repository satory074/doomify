...
- フィード: なじみ(保存曲・トップ曲・最近再生・自分のプレイリスト)+ 発見(似たアーティスト・参加作品・アルバム深掘り・ジャンル×年代検索・新譜)を「発見度」スライダーで配合。
  並びは TikTok / Instagram Reels / X の For You と同じ仕組み: なじみ・期待値・試験(未知アーティスト)・くじ引きの枠を混ぜ、行動(完走・保存・共有・スキップ…)の確率の重み付き和で候補を採点し、同じアーティストの連続を減衰させ、このセッションで長く聴いたタグ・アーティストに寄せていきます
- 発見は「まだ知らない曲」だけ: よく聴く/保存/フォロー中のアーティストや保存済みの曲は発見として出しません。カードには「〇〇が好きなら」「『曲』に似た曲」のように理由が付きます
- 学習: 滞在時間・完走率 / 戻ってきた / いいね / プレイリスト / 共有 / 「こういうのをもっと」「これは違う」を手応えとして、アーティスト・ジャンル・発見源ごとの当たりやすさを学び、発見度もスライダー ±15% の範囲で自動調整します。
  未知のアーティストはまず 1 曲だけ試し、反応が良ければ 2 曲 → 4 曲と広げます。「これは違う」はそのアーティストを 7 日避け、まだ表示していない先のカードも引き直します(設定の「発見の様子」で確認できます)
...
- 操作: いいね(♡ / アートを 2 回タップ)、プレイリストに追加、共有(共有シート / リンクのコピー)、Spotify で開く、「こういうのをもっと」「これは違う」
